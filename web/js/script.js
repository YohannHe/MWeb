;(function ($) {
    ...
    $(window).bind('resize', function () {
        reset();
        if(document.getElementById('container').offsetWidth<640){
            var w = document.getElementById('loading_dark').offsetWidth-250;
            document.getElementById('menu').style.left=w+"px";
        }else{
            var w = (document.getElementById('loading_dark').offsetWidth-640)/2;
            document.getElementById('menu').style.left=(640+w-250)+"px";
        }
        $('.works').each(function () {
    ...
    function tcst() {
        ...
        $('#list li').bind('touchstart touchend', function (e) {
    ...
    tcst();
    $('.con ul li').bind('touchstart touchend', function (e) {
        if (e.type == 'touchstart') {
            $(this).addClass('touch');
        } else if (e.type == 'touchend') {
            $(this).removeClass('touch');
    ...
    $('.search_open').bind('click', function (e) {
        ...
        if( window.location.href.split("?").length>1){
            var index = window.location.href.split("?")[0].split("/");
            if (index[index.length-1].toString()!="index.php"){
            ...
            var li=document.getElementById("menuList").getElementsByTagName("a");
            for (var i = 1 ;i<li.length;i++){
                var marr = li[i].href.split("?")[1].split("/");
                var mar = marr[marr.length-1];
                var ma = mar.split("&");

                if(u1[0]== ma[0]){
                    $(li[i].parentNode).addClass("menu_cur");
                }else {
                    $(li[i].parentNode).removeClass("menu_cur");
        ...
        $("#loading_dark").bind('touchmove', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
        });
    ...
    $.fn.openNewFrame = function () {
        ...
        $(this).click(function () {
            ...
            if (document.getElementById('newwrap')) {
                //$(window.parent.document).bind('touchmove', function (e) {
                //    e.preventDefault();
                //    e.stopImmediatePropagation();
                //});
